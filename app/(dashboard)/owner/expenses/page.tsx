...
type Search = { date_from?: string; date_to?: string; category?: string; source?: string };
...
export default async function Page({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  let query = supabase.from('expenses').select('id, expense_time, description, category, amount, payment_method, source, note').order('expense_time', { ascending: false }).limit(120);
  ...
  return (
    <DashboardShell role="owner" title="Owner expenses" description="Capture business expenses and monitor chef vs owner spending.">
      ...
      <form className="mt-6 grid gap-2 rounded border p-3 md:grid-cols-5">
