...
export default async function Page({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  if (params.sale_id) {
    const { data } = await supabase
      .from('sales')
      .select('id, sale_number, sold_by, sold_at, subtotal, total, payment_method, note')
    ...
    const { data: items } = await supabase
      .from('sale_items')
      .select('id, quantity, unit_price, line_total, menu_items(name)')
  ...
  return (
    <DashboardShell role="owner" title="Owner sales" description="Review sales by date, waiter, and payment method.">
      <form className="mb-4 grid gap-2 rounded border p-3 md:grid-cols-5">
        ...
        <select name="payment_method" defaultValue={params.payment_method ?? ''} className="rounded border px-2 py-1 text-sm">
      ...
      <div className="space-y-2">
        {(sales ?? []).length === 0 ? (
          <p className="rounded border border-dashed p-4 text-sm text-slate-500">No sales found for selected filters.</p>
        ) : (
          (sales ?? []).map((sale: any) => (
            <Link
              key={sale.id}
              href={`/owner/sales?sale_id=${sale.id}${params.date_from ? `&date_from=${params.date_from}` : ''}${params.date_to ? `&date_to=${params.date_to}` : ''}${params.waiter ? `&waiter=${params.waiter}` : ''}${params.payment_method ? `&payment_method=${params.payment_method}` : ''}`}
              className="block rounded border p-3 text-sm hover:border-black"
            >
      ...
      {selectedSale ? (
        <section className="mt-6 space-y-2 rounded-lg border p-4">
          <h2 className="text-base font-semibold">Sale detail: {selectedSale.sale_number}</h2>
          <p className="text-sm text-slate-600">
            {new Date(selectedSale.sold_at).toLocaleString()} · {selectedSale.payment_method} · Waiter: {waiterNameMap.get(selectedSale.sold_by) ?? 'Unknown'}
          </p>
          {selectedSale.note ? <p className="text-sm">Note: {selectedSale.note}</p> : null}
          <div className="space-y-1 text-sm">
            {saleItems.map((item: any) => (
              <p key={item.id} className="flex justify-between rounded border px-2 py-1">
