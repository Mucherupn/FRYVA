...
export default async function Page({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  let query = supabase.from('purchases').select('id, purchase_date, item_name, category, qty, unit, total_cost, payment_method, supplier, note').order('purchase_date', { ascending: false }).limit(120);
  ...
  return (
    <DashboardShell role="owner" title="Owner purchases" description="Record daily purchases and track outflows by category/date.">
      <PurchasesWorkflow defaultDate={today} />
      ...
      <section className="mt-3 space-y-2">
        {(purchases ?? []).map((purchase) => (
          <article key={purchase.id} className="rounded border p-3 text-sm">
            ...
            <p className="text-xs text-slate-500">{purchase.purchase_date} · {purchase.payment_method}{purchase.category ? ` · ${purchase.category}` : ''}{purchase.supplier ? ` · ${purchase.supplier}` : ''}</p>
          </article>
        ))}
      </section>
    </DashboardShell>
