...
export default async function OwnerDashboardPage() {
  ...
  const [ledgerResult, debtResult, salesResult, expensesResult, purchasesResult, openingResult, productionResult, trackedItemsResult, soldResult, purchasedTodayResult] = await Promise.all([
    ...
    supabase.from('purchases').select('item_name, qty').eq('purchase_date', today),
  ]);
  ...
  for (const row of purchasedTodayResult.data ?? []) purchasedByName.set(row.item_name.trim().toLowerCase(), (purchasedByName.get(row.item_name.trim().toLowerCase()) ?? 0) + Number(row.qty));
  ...
  const stockSummary = (trackedItemsResult.data ?? []).map((item) => {
    ...
    const purchased = purchasedByName.get(item.name.trim().toLowerCase()) ?? 0;
  ...
  return (
    <DashboardShell role="owner" title="Owner dashboard" description="Daily operational finance and stock visibility for business control.">
      ...
      <section className="mt-6 space-y-2">
        ...
        <p className="text-xs text-slate-500">Estimated closing = opening + production + purchases (matched by item name) - sold quantity.</p>
