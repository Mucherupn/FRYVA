...
export default async function OwnerDashboardPage() {
  ...
  const [ledgerResult, debtResult, salesResult, staffMealsResult, expensesResult, purchasesResult, openingResult, productionResult, closingResult, trackedItemsResult, soldResult, purchasedTodayResult, reviewsResult] = await Promise.all([
    ...
    supabase.from('sales').select('total, payment_method').gte('sold_at', todayRange.from).lt('sold_at', todayRange.to).eq('status', 'finalized'),
    supabase.from('staff_meal_records').select('total_value').eq('business_date', today),
    ...
    supabase.from('closing_stock_entries').select('menu_item_id, qty').eq('entry_date', today),
    ...
    supabase.from('stock_variance_reviews').select('id,status').eq('business_date', today),
  ]);
  ...
  const cashSales = (salesResult.data ?? []).filter((row: any) => row.payment_method === 'cash').reduce((sum, row) => sum + Number(row.total), 0);
  const mpesaSales = (salesResult.data ?? []).filter((row: any) => row.payment_method === 'mpesa').reduce((sum, row) => sum + Number(row.total), 0);
  const todaySales = cashSales + mpesaSales;
  const staffMealValue = (staffMealsResult.data ?? []).reduce((sum, row) => sum + Number(row.total_value), 0);
  ...
  for (const row of closingResult.data ?? []) closingByItem.set(row.menu_item_id, Number(row.qty));
  let stockAlertsToday = 0;
  ...
  const stockSummary = (trackedItemsResult.data ?? []).map((item) => {
    ...
    const actualClosing = closingByItem.get(item.id);
    if (actualClosing !== undefined && actualClosing !== estimatedClosing) stockAlertsToday += 1;
    return { item: item.name, opening, production, sold, purchased, estimatedClosing, actualClosing };
  });
  ...
  return (
    <DashboardShell role="owner" title="Owner mission control" description="Live financial and operational visibility with owner-grade controls.">
      <section className="kpi-grid">
        ...
        <MetricCard label="Cash today" value={money(cashSales)} />
        <MetricCard label="Mpesa today" value={money(mpesaSales)} />
        <MetricCard label="Outstanding debt" value={money(outstandingDebt)} />
        <MetricCard label="Staff meal value today" value={money(staffMealValue)} />
        <MetricCard label="Today expenses" value={money(todayExpenses)} />
        <MetricCard label="Today purchases" value={money(todayPurchases)} />
        <MetricCard label="Stock alerts today" value={String(stockAlertsToday)} />
      ...
      <section className="panel">
        ...
        <p className="section-subtitle">Estimated closing = opening + production + purchases − sold quantity. Staff meals are tracked separately in Stock Alerts.</p>
        {stockSummary.length === 0 ? <EmptyState title="No tracked stock today" description="Opening stock and production entries will appear here." /> : (
          <div className="table-shell" style={{ marginTop: 12 }}>
            <table className="table">
              <thead>
                <tr><th>Item</th><th className="money">Opening</th><th className="money">Production</th><th className="money">Purchased</th><th className="money">Sold</th><th className="money">Estimated closing</th><th className="money">Actual closing</th></tr>
              </thead>
              <tbody>
                {stockSummary.map((row) => (
                  <tr key={row.item}><td>{row.item}</td><td className="money">{row.opening}</td><td className="money">{row.production}</td><td className="money">{row.purchased}</td><td className="money">{row.sold}</td><td className="money">{row.estimatedClosing}</td><td className="money">{row.actualClosing ?? '—'}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
